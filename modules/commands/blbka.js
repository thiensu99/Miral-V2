...
module.exports.handleEvent = function({ api, event, args, Threads }) {
  ...
  var tl = ["chào bạn tôi là bot của Sứ", "bạn kêu tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~", "Yêu thương admin nhất", "Anh ấy là phụ trợ của admin", "Sao thế công chúa", "Đừng làm em đau ~~~~", "Rên với em nhé a á á á", "Tuyển phi công nè ạ", "Làm đĩ ko ạ? dui lắm", "Cậu cô đơn ko?", "Chịch ko ạ vã quá!!!", "Được của ló :)))", "Em dthw như chủ của em ạ", "Đừng khen em ngại quá hí hí" ,"Làm chồng em ko ạ?", "Đừng spam em nha :<<, cậu chủ em mệt lắm ời", "Cút ra", "Đừng spam em lag!!!", "Đút tutu thôi em đau :'(", "Yêu cậu như một cực hình\nNhấp lên nhấp xuống hai mình cùng rên", "Spam cc cút", "Yêu em ko?", "Chồng em đây rồi"];
  var rand = tl[Math.floor(Math.random() * tl.length)]

  if ((event.body.toLowerCase() == "bot ngu")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Đinh Thiên Sứ để được hỗ trợ\nLink fb nè: https://www.facebook.com/HackingOfficiall2", threadID, () =>
  ...
  if ((event.body.toLowerCase() == "anh sứ") || (event.body.toLowerCase() == "anh Sứ")) {
  ...
  if ((event.body.toLowerCase() == "a sứ") || (event.body.toLowerCase() == "a sứ")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "sứ") || (event.body.toLowerCase() =="Sứ")) {
    return api.sendMessage("Anh ấy tuy ko đẹp trai hay giỏi gì nhưng được cái hài hước và luôn mong đạt được mục tiêu anh ấy muốn <3 <3 ", threadID);
  };

  if ((event.body.toLowerCase() == "@Sứ") || (event.body.toLowerCase() == "@Gà")) {
    return api.sendMessage("Ai kêu tao đấy, mà làm ơn đừng tag bot mà hãy liên hệ admin qua Fb: https://www.facebook.com/HackingOfficiall2 ", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "Alo") || (event.body.toLowerCase() == "alo")) {
    return api.sendMessage("Lô cc :))", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao tên Sứ có gì dùng lệnh /ad hoặc /info admin để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "ông Sứ") || (event.body.toLowerCase() == "ong Sứ")) {
    return api.sendMessage("️Kêu chủ của tôi gì á :)?", threadID);
  };

  if ((event.body.toLowerCase() == "ông sứ") || (event.body.toLowerCase() == "Ông sứ")) {
  ...
  if ((event.body.toLowerCase() == "haha") || (event.body.toLowerCase() == "haha")) {
    return api.sendMessage("️Haha dui quá =>>", threadID);
  };
...
if ((event.body.toLowerCase() == "Tvm") || (event.body.toLowerCase() == "tvm")) {
    return api.sendMessage("Gt + set bd nha <3", threadID);
  };
