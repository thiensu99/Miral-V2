module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Quốc Anh",
    description: "Lấy avatar người dùng/nhóm.",
...
module.exports.run = async({api,event,args,Users}) => {
    ...
    const prefix = (threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX;
     if (args.length == 0) return api.sendMessage(`➩ Bạn có thể dùng:\n\n${prefix}${this.config.name} user => Bot sẽ lấy avt của chính bạn.\n\n${prefix}${this.config.name} user @tag => Bot sẽ lấy avt người bạn tag.\n\n${prefix}${this.config.name} box => Bot sẽ lấy avt box của bạn\n\n${prefix}${this.config.name} [user/tid] => Bot sẽ lấy avt của [user/tid] đó`, event.threadID, event.messageID);
    if (args[0] == "box") {
